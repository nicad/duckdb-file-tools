...
fn age_encrypt_multi(data: &[u8], recipient_strs: &[String]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
fn age_decrypt_multi(data: &[u8], identity_strs: &[String]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    let decryptor = age::Decryptor::new(data)
        .map_err(|e| format!("Failed to create age decryptor: {}", e))?;
